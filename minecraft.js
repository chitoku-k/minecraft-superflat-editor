var world = { blocks: [], error: [] };
var draggingBlock, droppedPosition, dragFrom, dragTo;
var isIE;
/*@cc_on
	if (@_jscript_version < 10 || typeof history.pushState === "undefined")
		isIE = true;
@*/
...
String.prototype.toHalfbyte = function () {
...
function createBlockTable(array) {
	...
	if (isIE)
		$("blockTable").outerHTML = "<table id=\"blockTable\" class=\"stripeTable\">" + blocks + "</table>";
	else
		$("blockTable").innerHTML = blocks;

	if ('\v' == 'v') {
		var columns = $("blockTable").getElementsByTagName("tr");
		for (i = 0, n = columns.length; i < n; i++) {
			if (i % 2)
				columns[i].className = "stripeTableOdd";
		}
	}
}
...
function createWorldTable() {
	...
	if (isIE)
		$("worldTable").outerHTML = "<table id=\"worldTable\" class=\"stripeTable\" ondragover=\"cancelDrop(event)\">" + blocks + "</table>";
	else
	...
	$("dungeonOption").checked = world.dungeon ? true : false;
	$("decorationOption").checked = world.decoration ? true : false;
	$("lakeOption").checked = world.lake ? true : false;
	$("lava_lakeOption").checked = world.lava_lake ? true : false;
...
function searchBlock() {
	var name = this.value.toKatakana().toHalfbyte().toLowerCase();
	...
	for (var i = 0, n = Minecraft.blocks.length; i < n; i++) {
		if (Minecraft.blocks[i] instanceof Array) {
			for (var j = 0, m = Minecraft.blocks[i].length; j < m; j++) {
				...
				if (perfectMatch) {
					if (ja_name && ja_name.toKatakana().toHalfbyte().toLowerCase() == name)
						array.push(Minecraft.blocks[i][j]);
					else if (en_name && en_name.toLowerCase() == name)
						array.push(Minecraft.blocks[i][j]);
				}
				else {
					if (ja_name && ja_name.toKatakana().toHalfbyte().toLowerCase().indexOf(name) >= 0)
						array.push(Minecraft.blocks[i][j]);
	...
	if (array.length)
		...
		if (isIE)
			$("blockTable").outerHTML = "<table id=\"blockTable\" class=\"stripeTable\"><tr><td style=\"text-align: center;\">ブロックがありません</td></tr></table>";
		else
...
function blockDragStart(event) {
	/*  Supports for Firefox D&D events.
		(It causes crash on Opera.) */
	if (!window.opera)
...
function parsePresetCode(code) {
	...
	if (customStrings[1].match(/[\dx,:]+/)) {
		...
		for (i = 0; i < blocks.length; i++) {
			if (blocks[i].match(/^([\d]+)x([\d:]+)$/)) {
				if (getBlock(RegExp.$2))
					world.blocks.push({
						block: getBlock(RegExp.$2),
						length: RegExp.$1
					});
			}
			else if (blocks[i].match(/^([\d:]+)$/)) {
				if (getBlock(RegExp.$1))
					world.blocks.push({
						block: getBlock(RegExp.$1),
						length: 1
					});
			}
			else
	...
	for (i = 0; i < structures.length; i++) {
		if (structures[i].match(/^(dungeon|decoration|lake|lava_lake)$/))
			world[RegExp.$1] = true;
		else if (structures[i].match(/^village(?:\(([a-z0-9\.=\s]+)\))?$/)) {
			var village = RegExp.$1;
			world.village = {};
			if (village.length > 0) {
				var option = village.split(" ");
				for (j = 0; j < option.length; j++) {
					if (option[j].match(/^distance=([\d\.]+)$/))
						world.village.distance = RegExp.$1;
					else if (option[j].match(/^size=([\d\.]+)$/))
						world.village.size = RegExp.$1;
					else
						world.error.push("'" + option[j] + NOT_SUPPORTED);
				}
			}
		}
		else if (structures[i].match(/^mineshaft(?:\(([a-z0-9\.=\s]+)\))?$/)) {
			var mineshaft = RegExp.$1;
			world.mineshaft = {};
			if (mineshaft.length > 0) {
				var option = mineshaft.split(" ");
				for (j = 0; j < option.length; j++) {
					if (option[j].match(/^chance=([\d\.]+)$/))
						world.mineshaft.chance = RegExp.$1;
					else
						world.error.push("'" + option[j] + NOT_SUPPORTED);
				}
			}
		}
		else if (structures[i].match(/^stronghold(?:\(([a-z0-9\.=\s]+)\))?$/)) {
			var stronghold = RegExp.$1;
			world.stronghold = {};
			if (stronghold.length > 0) {
				var option = stronghold.split(" ");
				for (j = 0; j < option.length; j++) {
					if (option[j].match(/^distance=([\d\.]+)$/))
						world.stronghold.distance = RegExp.$1;
					else if (option[j].match(/^count=([\d\.]+)$/))
						world.stronghold.count = RegExp.$1;
					else if (option[j].match(/^spread=([\d\.]+)$/))
						world.stronghold.spread = RegExp.$1;
					else
						world.error.push("'" + option[j] + NOT_SUPPORTED);
				}
			}
		}
		else if (structures[i].match(/^biome_1(?:\(([a-z0-9\.=\s]+)\))?$/)) {
			var biome_1 = RegExp.$1;
			world.biome_1 = {};
			if (biome_1.length > 0) {
				var option = biome_1.split(" ");
				for (j = 0; j < option.length; j++) {
					if (option[j].match(/^distance=([\d\.]+)$/))
						world.biome_1.distance = RegExp.$1;
					else
						world.error.push("'" + option[j] + NOT_SUPPORTED);
				}
			}
...
addEvent(window, "load", function () {
	...
	if (isIE)
		$("presetWindowTable").outerHTML = "<table id=\"presetWindowTable\" class=\"stripeTable listBoxWindowTable\">" + presets + "</table>";
	else
	...
	if (isIE)
		$("biomeWindowTable").outerHTML = "<table id=\"biomeWindowTable\" class=\"stripeTable listBoxWindowTable\">" + biomes + "</table>";
	else
	...
	if (!window.clipboardData) {
		...
		var clip = new ZeroClipboard($("presetCopyButton"));
		...
		clip.on("dataRequested", function (client, args) {
			client.setText($("presetTextBox").value);
		});
	}
	else {
		addEvent($("presetCopyButton"), "click", function () {
			clipboardData.setData("text", $("presetTextBox").value);
			this.value = "コピー完了";
			setTimeout(function () {
				$("presetCopyButton").value = "コピー";
			}, 1000);
